...
function validateFile(filePath: string): Problem[] {
  ...
  try {
    ...
    if (err instanceof yaml.YAMLException) {
      return [{ line: err.mark?.line ? err.mark.line + 1 : undefined, message: `YAML parse error: ${err.reason}` }];
    }
  ...
  for (let i = 0; i < doc['rules'].length; i++) {
    const rule = doc['rules'][i] as Record<string, unknown>;
...
export function runValidate(args: string[]): void {
  ...
  if (files.length === 0) {
    // Default: validate both bundled rule files
    const projectRoot = process.env.AG_HOME ?? resolve(new URL(import.meta.url).pathname, '..', '..', '..');
