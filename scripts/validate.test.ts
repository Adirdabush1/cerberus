...
function run(file: string): { code: number; out: string } {
  try {
    const out = execSync(`node_modules/.bin/tsx src/cli/index.ts rules validate --file ${file}`, {
...
check('broken file reports duplicate id', r2.out.includes('duplicate'), r2.out.toLowerCase());
...
try {
  const bundledOut = execSync('node_modules/.bin/tsx src/cli/index.ts rules validate', {
    encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'],
  });
